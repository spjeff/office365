...
if ($SearchServiceAccount.Contains("\")) {      
    # if True then domain\username was used
    $Account = $SearchServiceAccount.Split("\")      
    $Account = $Account[1]      
} else {
    # no domain was specified at account entry 
...
if ($results.Count -gt 0) {       
    ...
    do {
        Write-Output .
        Start-Sleep 10
    } while (($searchApp.GetTopology($topology.TopologyId).State -ne "Active") -and ($timeoutTime -ge (Get-Date)))      
    ...
    # The Account Must Exist so we can proceed with the script
